...
* Set everything up
** package.el (load first)
Load package.el before any other code so use-package and packages load correctly; then straight.el loads after.
...
#+begin_src emacs-lisp
  ...
  (custom-set-variables
   ...
   '(org-agenda-files '("~/Dropbox/org/Tasks.org" "~/Dropbox/org/Cumpleaños.org")))
#+end_src

** Use-package
Better package management for emacs. use-package and packages from ELPA; packages with ~:straight~ from straight.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  ;; (defun my/python-send-line-and-move ()
...
#+begin_src emacs-lisp
  ...
  (defun skls/org-babel-tangle-config ()
    (when (and (buffer-file-name)
               (locate-file "Emacs.org" (list (file-name-directory (buffer-file-name)))))
...
#+BEGIN_SRC emacs-lisp
  (when (equal system-type 'darwin)
    ...
    (setenv "PATH" (concat (getenv "PATH") ":/usr/local/bin/:$HOME/.pyenv/shims"))
    (setenv "PATH" (concat (getenv "PATH") ":/usr/local/bin/:/opt/homebrew/bin/:/opt/homebrew/sbin/:/opt/homebrew/opt/libpq/bin:$HOME/.local/bin/"))
